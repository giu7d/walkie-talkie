defmodule WalkieTalkieWeb.PeerChannel do
  ...
  def join("room:" <> room_id, params, socket) do
    ...
    case :global.whereis_name(room_id) do
      :undefined ->
        WalkieTalkie.Room.start(%{room_id: room_id, simulcast?: simulcast?},
          name: {:global, room_id}
        )
  ...
  defp do_join(socket, room_pid, room_id, simulcast?) do
    ...
    try do
      WalkieTalkie.Room.add_peer_channel(room_pid, self(), peer_id)
    catch
      :exit, _reason ->
        Logger.info("Room #{inspect(room_id)} died when trying to join. Creating a new one.")

        %{room_id: room_id, simulcast?: simulcast?}
        |> WalkieTalkie.Room.start(name: {:global, room_id})
        |> handle_start_room_result(socket, room_id, simulcast?)
    else
      :ok ->
