defmodule WalkieTalkie.Room do
  ...
  def init(args) do
    ...
    turn_mock_ip = Application.fetch_env!(:walkie_talkie, :integrated_turn_ip)
    ...
    turn_cert_file =
      case Application.fetch_env(:walkie_talkie, :integrated_turn_cert_pkey) do
        {:ok, val} -> val
        :error -> nil
      end
    ...
    integrated_turn_options = [
      ...
      ports_range: Application.fetch_env!(:walkie_talkie, :integrated_turn_port_range),
      cert_file: turn_cert_file
    ]

    network_options = [
      integrated_turn_options: integrated_turn_options,
      integrated_turn_domain: Application.fetch_env!(:walkie_talkie, :integrated_turn_domain),
      dtls_pkey: Application.get_env(:walkie_talkie, :dtls_pkey),
      dtls_cert: Application.get_env(:walkie_talkie, :dtls_cert)
    ]

    tcp_turn_port = Application.get_env(:walkie_talkie, :integrated_tcp_turn_port)
    TURNManager.ensure_tcp_turn_launched(integrated_turn_options, port: tcp_turn_port)

    if turn_cert_file do
      tls_turn_port = Application.get_env(:walkie_talkie, :integrated_tls_turn_port)
    ...
    {:ok, _pid} =
      DynamicSupervisor.start_child(
        WalkieTalkie.RoomMonitorSupervisor,
        {WalkieTalkie.Room.Monitor, [self(), room_id]}
      )
